...
const toppingsDivs = document.querySelectorAll('.topping')
...
const toggleClass = (item, c) => {
    item.classList.contains(c) ? item.classicList.remove(c) : item.classList.add(c)
}
const camelize = str => {
...
crustDivs.forEach(div => {
    div.addEventListener('click', e => {
        ...
        selectedCrust = e.target.inner.innerText
        updateBill()
    })
})
...
const updateBill = () => {
    ...
    // GRAND COST
    grandCost = gstCost + cheeseCost + toppingCost + totalCost
...
function fb() {s
    alert("your pizza(s) is arriving 30 mins. Have a nice day");
}
function bb() {
    alert("Our delivery charge is ksh500 ");
    alert("Please pin your exact location on the map below ");
}
...
$(document).ready(function() {
    $("submit").click(function(event) {
        ...
        document.getElementById("form1").requestFullscreen();
    });
});
